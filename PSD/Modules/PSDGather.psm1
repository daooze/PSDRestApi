...
Function Get-PSDSettings {
	...
	Process {
        ...
        if ($section.Contains("RestURI")) {
            Write-PSDLog "$($MyInvocation.MyCommand.Name): Found RestURI: '$($section["RestURI"])'"

            try {
                $param = Convert-IniSectionToRestApiParam -Section $section
                $section_tmp = Invoke-PSDGatherRestApi @param | Convert-RestApiResult
            }
            catch {
                Write-PSDLog -Message "$($MyInvocation.MyCommand.Name): $($_.Exception.Message)"
                Show-PSDInfo -Message "Unable to retrieve data from RestAPI. The system has logged the following error:`r`n$($_.Exception.Message)" -Severity Error -OSDComputername $OSDComputername -Deployroot $global:psddsDeployRoot
                throw
            }

            if ($section_tmp -ne $null) {
                Get-PSDSettings $section_tmp
                $section_tmp = $null
            }
            $skipProperties = $true
        }
